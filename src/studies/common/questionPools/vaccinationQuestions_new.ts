...
import { text_how_answer, text_select_all_apply, text_why_asking } from "./helpers";
...
import { ClientExpression, GroupQuestion } from "../../../tools";
...
export class VacGroup extends GroupQuestion {
...
export class FluVaccineThisSeason extends ItemQuestion {
    ...
    createNotVaccinatedCondition(): Expression {
        return ClientExpression.singleChoice.any(this.key, ResponseEncoding.flu_vaccine_season.no);
    }
...
interface FluVaccineWhenProps extends ItemProps {
 ...
 *
 */
export class FluVaccineThisSeasonWhen extends ItemQuestion {

    minDate?: ExpressionArg;

    constructor( props: FluVaccineWhenProps) {
        super(props, 'Q10b');
 ...
 */
export class FluVaccineThisSeasonReasonFor extends ItemQuestion {
 ...
 *
 */
export class FluVaccineThisSeasonReasonAgainst extends ItemQuestion {
...
export class CovidVac extends ItemQuestion {
    ...
    createIsVaccinatedCondition(): Expression {
        return ClientExpression.singleChoice.any(this.key, ResponseEncoding.covid_vac.yes);
    }

    createNotVaccinatedCondition(): Expression {
        return ClientExpression.singleChoice.any(this.key, ResponseEncoding.covid_vac.no);
    }
 ...
 */
export class CovidVaccineBrand extends ItemQuestion {
...
type KeyOfVaccineShotReponses = keyof typeof ResponseEncoding['covid_vac_shots'];

/**
 * VACCINE SHOTS: How many times has the participant been vaccinated
 */
export class CovidVaccineShots extends ItemQuestion {
    ...
    createOneShotCondition(): Expression {
        return ClientExpression.singleChoice.any(this.key, ResponseEncoding.covid_vac_shots.one);
    }

    createShotCondition(...responseKeys: KeyOfVaccineShotReponses[]): Expression {
        const rr = responseKeys.map(k => ResponseEncoding.covid_vac_shots[k]);
        return ClientExpression.singleChoice.any(this.key, ...rr);
    }
 ...
 */
export class CovidDateLastVaccine extends ItemQuestion {
...
type KeyOfSecondShotPlanResponse = keyof typeof ResponseEncoding.covid_second_shot_plan;

/**
 * SECOND SHOT: single choice question about second shot planned
 */
 export class CovidSecondShotPlan extends ItemQuestion {
    ...
    */
    ...
    getResponses(): OptionDef[] {
        const codes =  ResponseEncoding.covid_second_shot_plan;
        return [
            {
                key: codes.yes, role: 'option',
    ...
    createReponseCondition(...responseKeys: KeyOfSecondShotPlanResponse[]): Expression {
        const rr = responseKeys.map(k => ResponseEncoding.covid_second_shot_plan[k]);
        return ClientExpression.singleChoice.any(this.key, ...rr);
    }
 ...
 */
export class CovidSecondShotAgainstReason extends ItemQuestion {
 ...
 */
export class CovidVaccineProReasons extends ItemQuestion {
...
export class CovidVaccineAgainstReasons extends ItemQuestion {
